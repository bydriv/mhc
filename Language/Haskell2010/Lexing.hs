...
dfaInitialFinalStates = [2,3,4,5,6,7,8,9,13,14]
...
dfaDashesInitialState :: Int
dfaDashesInitialState = 1

dfaDashesFinalStates :: [Int]
dfaDashesFinalStates = [2,3]

dfaDashesTransition :: Int -> Char -> Int
dfaDashesTransition q c =
  let c' :: Int
      c' =
        case Char.ord c of
          10 -> 1
...
dfaNestedInitialState :: Int
dfaNestedInitialState = 1

dfaNestedFinalStates :: [Int]
dfaNestedFinalStates = [2,3,4]

dfaNestedTransition :: Int -> Char -> Int
dfaNestedTransition q c =
  let c' :: Int
      c' =
        case Char.ord c of
          45 -> 1
...
lex actions = lex' where
  lex' s = do
    ...
    if p == Initial then
      ...
      case match dfaDashesInitialState dfaDashesFinalStates dfaDashesTransition s of
        Nothing ->
          return ([], s)
        Just 0 ->
          return ([], s)
        Just i ->
          let (yytext, s') = splitAt i s in
            if match dfa41InitialState dfa41FinalStates dfa41Transition s == Just i then do
      ...
      case match dfaNestedInitialState dfaNestedFinalStates dfaNestedTransition s of
        Nothing ->
          return ([], s)
        Just 0 ->
          return ([], s)
        Just i ->
          let (yytext, s') = splitAt i s in
            if match dfa43InitialState dfa43FinalStates dfa43Transition s == Just i then do
              x <- saOpenNested actions yytext
              (xs, s'') <- lex' s'
              return (x : xs, s'')
