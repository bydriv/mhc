...
lex actions = lex' where
  lex' s = do
    ...
    if p == Initial then
      ...
      case max (match dfa124InitialState dfa124FinalStates dfa124Transition s, -124) $ max (match dfa125InitialState dfa125FinalStates dfa125Transition s, -125) $ max (match dfa126InitialState dfa126FinalStates dfa126Transition s, -126) $ max (match dfa127InitialState dfa127FinalStates dfa127Transition s, -127) $ max (match dfa128InitialState dfa128FinalStates dfa128Transition s, -128) $ max (match dfa129InitialState dfa129FinalStates dfa129Transition s, -129) $ (Nothing, 1 :: Int) of
        (Nothing, _) ->
          return ([], s)
        (Just 0, _) ->
          return ([], s)
